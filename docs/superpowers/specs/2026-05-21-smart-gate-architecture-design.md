...
Pi↔ESP32 application communication uses a **3-wire GPIO UART link**: Pi pin 8 (BCM14, TX0) → ESP32 **GPIO 32** (UART1 RX); Pi pin 10 (BCM15, RX0) ← ESP32 **GPIO 25** (UART1 TX); Pi pin 6 ↔ ESP32 GND. Both ends are 3.3 V CMOS so no level shifter is needed. The Pi serial console must be disabled in `raspi-config` so `/dev/serial0` is available to the daemon. A separate USB cable from the Pi to the DevKit's USB-C/micro-USB port is used **only for `esptool.py` firmware flashing** (the GPIO UART has no DTR/RTS to auto-trigger the bootloader); the cable can be unplugged once flashed. Application I/O on the Pi side opens with `dsrdtr=False, rtscts=False` and forces DTR/RTS low (harmless on `/dev/serial0`, important for the flash-time USB cable path on CP2102/CH340 DevKits — see `smart_gate/link/uart_client.py::_open_serial`). **No Wi-Fi on ESP32; no MQTT.** ESP32 maintains its own NVS-stored RFID allowlist so it can authenticate RFID and operate the barrier independently if the Pi link is lost.
...
- Physical (runtime): 3 Dupont leads between Pi GPIO header and ESP32 DevKit header.
  - Pi pin **8** (BCM14, UART0 TX) → ESP32 **GPIO 32** (UART1 RX, mapped via GPIO matrix)
  - Pi pin **10** (BCM15, UART0 RX) ← ESP32 **GPIO 25** (UART1 TX, mapped via GPIO matrix)
  - Pi pin **6** (GND) ↔ ESP32 GND
- Pi side: `/dev/serial0` (symlink to `/dev/ttyAMA0` PL011 on Pi 4, or `/dev/ttyS0` mini-UART depending on `dtoverlay`) at 115200 baud, 8N1, no flow control. The Pi serial console must be **disabled in `raspi-config`** (`Interface Options → Serial Port → No to login shell, Yes to serial hardware`) before the link works; the daemon user must be in group `dialout`.
- ESP32 side: UART1 (`HardwareSerial(1)`) initialised with `Serial1.begin(115200, SERIAL_8N1, PIN_PI_UART_RX=32, PIN_PI_UART_TX=25)`. RC522 keeps GPIO 5 (CS) and GPIO 17 (RST). UART0 (GPIO 1/3, via CP2102/CH340) remains available for `pio device monitor` debug log and for `esptool.py` flashing — it is not used for runtime app comm.
- `pyserial` open: `Serial(port, baud, timeout=1.0, dsrdtr=False, rtscts=False)` plus `ser.dtr = False; ser.rts = False`. The flags are harmless on `/dev/serial0` (PL011 has no DTR/RTS exposed) but are kept for the flash-time USB cable path so the daemon never auto-resets the ESP32 if someone temporarily routes app comm through USB-CDC. See `smart_gate/link/uart_client.py::_open_serial`.
- Voltage levels: both Pi UART and ESP32 GPIO are 3.3 V CMOS push-pull. No level shifter, no resistor divider needed. If the 3 Dupont leads are longer than ~50 cm, add a 1 kΩ pull-down on each RX line to reduce noise pickup.
- Firmware flashing: cable a USB-A↔USB-C between Pi and DevKit temporarily, run `sudo systemctl stop smart-gate` first, then `esptool.py --chip esp32 --port /dev/ttyUSB0 --baud 921600 write_flash 0x0 firmware.bin`, then `systemctl start`. The USB cable can be unplugged once flashed; runtime traffic is on the GPIO UART. Optionally keep the USB cable plugged in (VBUS cut or with the DevKit on its own 5 V supply) for `pio device monitor` debug log streaming.
